...
## Ambiguity normality check and transformation
...
```{r}
d %>% 
  ggplot(aes(x = tidytext_ambi %>% log)) +
  geom_histogram(bins=250)
```
Most normally distributed when log() applied
```{r}
d %>% 
  ggplot(aes(x = tidytext_ambi %>% sqrt)) +
  geom_histogram(bins=150)
```
### SentiStrength

Always homomorphic, decreases left to right
```{r}
d %>% 
  ggplot(aes(x = ss_ambi %>% sqrt)) +
  geom_histogram()
```
...
```{r}
d %>% 
  ggplot(aes(x = liwc_ambi %>% log)) +
  geom_histogram()
```
Sqrt
```{r}
d %>% 
  ggplot(aes(x = liwc_ambi %>% sqrt)) +
  geom_histogram()
```
